...
# 引入本地缓存模块
try:
    from stock_cache import get_cached_price, save_daily_data_to_cache, init_stock_cache_table
    USE_CACHE = True
except ImportError:
    USE_CACHE = False
...
def get_price_day_tx(code, end_date='', count=10, frequency='1d'):     #日线获取
    ...
    # 补充今天实时数据（如果今天是交易日且当前时间在交易时间内）
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    stock_name = ''  # 用于保存股票名称
    
    if today not in df.index.strftime('%Y-%m-%d').tolist():
        try:
            # 使用腾讯实时行情接口获取今天的数据
            rt_url = f'http://qt.gtimg.cn/q={code}'
            rt_text = requests.get(rt_url, timeout=5).text
            if '~' in rt_text:
                rt_data = rt_text.split('~')
                if len(rt_data) > 40:
                    rt_open = float(rt_data[5]) if rt_data[5] else 0
                    rt_close = float(rt_data[3]) if rt_data[3] else 0
                    rt_high = float(rt_data[33]) if rt_data[33] else 0
                    rt_low = float(rt_data[34]) if rt_data[34] else 0
                    rt_volume = float(rt_data[6]) * 100 if rt_data[6] else 0  # 手转股
                    stock_name = rt_data[1] if len(rt_data) > 1 else ''  # 股票名称
            ...
            pass  # 实时数据获取失败，只返回历史数据

    # 保存到本地缓存（带股票名称）
    if USE_CACHE and df is not None and not df.empty:
        try:
            save_daily_data_to_cache(code, df, name=stock_name)
        except Exception as e:
            print(f"⚠️  缓存保存失败: {e}")
...
def get_price_min_tx(code, end_date=None, count=10, frequency='1d'):    #分钟线获取 
    ...
    df.time=pd.to_datetime(df.time);   df.set_index(['time'], inplace=True);   df.index.name=''          #处理索引     
    # 避免 chained assignment / 位置索引弃用告警（pandas 3.0 兼容）
...
def get_price(code, end_date='',count=10, frequency='1d', fields=[]):        #对外暴露只有唯一函数，这样对用户才是最友好的
    ...
    if  frequency in ['1d','1w','1M']:   #1d日线  1w周线  1M月线
         ...
         if USE_CACHE:
             try:
                 df_cached = get_cached_price(xcode, end_date=end_date, count=count, frequency=frequency, 
                                             use_api=False, api_get_price=None)
                 if df_cached is not None and not df_cached.empty and len(df_cached) >= count:
                     return df_cached
             except:
                 pass
         
         # 缓存不足，调用 API（腾讯优先）
         try:    return get_price_day_tx(xcode,end_date=end_date,count=count,frequency=frequency)   #主力：腾讯财经
         except: return get_price_sina( xcode, end_date=end_date,count=count,frequency=frequency)   #备用：新浪
    ...
    if  frequency in ['1m','5m','15m','30m','60m']:  #分钟线 ,1m只有腾讯接口  5分钟5m   60分钟60m
         ...
         try:    return get_price_day_tx( xcode,end_date=end_date,count=count,frequency=frequency)   #主力：腾讯
